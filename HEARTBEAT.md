...
> - 2026-04-25 15:09 - ✅ 后端 DTO 一致性续:7 个 Response DTO 新增 equals/hashCode/toString——ApiSloComplianceResponse(3 records:outer+EndpointSlo+LatencyStats)/BatchCreateAndEmbedResponse+DocumentResult inner/BatchCreateResponse+DocumentResult inner/BatchEmbedResponse(3 records:outer+BatchEmbedResultItem+BatchEmbedSummary)/ChatHistoryResponse/CollectionCreatedResponse/CollectionDeleteResponse;Double.compare 处理浮点字段(LatencyStats/EndpointSlo);DtoTest +46 tests(equals same/different/null/typeCheck + toString);api module 434 tests;mvn test ✅(exit 0);commit de632c7 已推送
