> - 2026-04-25 13:47 - ✅ 后端 DTO 一致性续:6 个 Response/Request record 新增 equals/hashCode/toString——AlertActionResponse(2字段)/VariantResponse(1)/BatchDeleteResponse(2字段含List+inner record)/FireAlertResponse(Long+String)/ResolveAlertRequest(1字段)/ReembedMissingResponse(4字段含List);DtoTest +24 tests(4 per DTO:equals same/different/null+typeCheck + toString);35→29个 DTO 仍待处理;mvn test ✅(exit 0);commit 5c9a9d5 已推送
