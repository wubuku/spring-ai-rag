...
## 7. 日期键盘录入 follow-up（2026-08-15）

### 7.1 问题与根因

真实浏览器中逐位输入年份时，`datetime-local` 会在分段值尚未完整期间暂时向页面暴露
空字符串。当前 React 组件把该字段实现为受控输入：
...
React 收到中间空值后立即回写 DOM，导致浏览器尚未完成的年份分段被清空。例如慢速输入
`2099` 时可能只剩 `99`；快速输入则可能在状态回写前完成，因此表现不稳定。

### 7.2 实施方案

- 将 expiry 字段改为 `defaultValue` 驱动的非受控原生 `datetime-local`。
- 提交时从当前 `<form>` 的 `FormData` 读取最终 `expiresAt`，不依赖逐键 React 状态。
- 可保留仅用于按钮状态的布尔值，但不得再通过 React `value` 属性回写日期内容。
- 继续保留 `required`、动态 `min`、默认一年和无固定 `max` 的现有行为。
- 增加回归测试，验证日期字段经历原生分段输入的中间空值和组件重渲染后，最终
  `2099` 年值不会被 React 覆盖，并按原值提交。

### 7.3 验收结果

- Vitest 已覆盖非受控日期值在重渲染后保持、长期日期提交和空值阻断；前端全量
  `163/163` 通过。
- Chromium Mock E2E 以每字符 250ms 的速度向年份分段输入 `2099`，控件值和创建请求
  均保留完整 `2099-12-31T23:59`，用例 `1/1` 通过。
- 正在运行的 Vite 页面额外验证了中间值 `0002 -> 0020 -> 0209 -> 2099`；随后修改名称
  触发 React 重渲染，年份仍保持 `2099`。
- ESLint、TypeScript `--noEmit` 和 production build 均通过。
- Maven `-Pwebui generate-resources` 已重新生成并同步后端内嵌 WebUI；`dist` 与后端
  静态目录入口一致。
- 基础门槛后连续三轮固定范围检查均未发现问题、未修改代码，计数 `3 / 3`。
