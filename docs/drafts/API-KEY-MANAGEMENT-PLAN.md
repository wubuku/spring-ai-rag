...
> 起草日期：2026-04-15  
> 状态：历史规划；记录当前 V22–V24 方案的来源，不再作为外部服务加固目标
> 替代规划：[API Key 加固独立实施规划](2026-08-14_API_KEY_HARDENING_IMPLEMENTATION_PLAN.md)
> 注意：本文中的 NORMAL 自助创建、启动日志输出 ADMIN secret、单表轮换等结论只用于
> 理解历史实现，不得用于新的生产外部凭据系统。
