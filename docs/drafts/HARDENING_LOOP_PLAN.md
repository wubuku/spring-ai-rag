...
- 分支：`test/coverage-batch65-20260906`（基于 main@b469c345）
- 内容：Documents edit 保存流深度交互测试（实际 3 个测试均在
  Documents.interactions.test.tsx 中）——覆盖编辑对话框加载详情、
  revision CAS 提交、cancel 不触发 update、loaded-src 回填断言。

## 10. 下一批次入口（候选，按优先级）

- Batch 66：Evaluation citations tab 测试（Batch 54 已完成）。
- Batch 67：后续数据驱动 coverage 找新薄弱点。