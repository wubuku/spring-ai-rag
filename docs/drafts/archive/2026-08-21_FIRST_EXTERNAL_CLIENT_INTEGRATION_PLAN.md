...
> 长青文档。
>
> 可恢复进度：[对应历史进度](2026-08-21_FIRST_EXTERNAL_CLIENT_INTEGRATION_PROGRESS.md)
...
scenemill 仓库的远端历史分支 `origin/feature/generated-video-asset-rag-20260820` 确实存在；第一阶段收口提交为
`737d38e2d`。该实现尚未进入 scenemill `origin/main@71e8cda86`，但已被最新本地
`main@27871d057` 吸收。`web-studio` 的生成视频分页提交也已进入其远端
`main@2a288b98`。下一阶段专用分支
`codex/scenemill-first-rag-client-20260821@27871d057` 已从本地 main 创建、快进同步到当前最新本地
main 并推送，不再把新开发叠加到第一阶段历史分支。
...
- 专项 PostgreSQL、视频结果回归、clean compile、前端 build/Playwright 和隔离 V435 dev smoke
  已通过；收口提交 `737d38e2d` 中的进度账本记录调度池从 22 修正为 27 后重新验证，并完成连续
  `3/3` 无实现修改检查。

因此本批实施的 Gate 0 不是重做旧基线审查，而是在新专用分支上验证本地 main 的最终组合。
第一阶段旧基线 `3/3` 和收口提交保留为历史证据，但不能直接替代 `27871d057` 组合的受影响门槛
与三轮限定范围检查。
...
- 使用已推送的 `codex/scenemill-first-rag-client-20260821@27871d057`，禁止回到第一阶段历史分支
  继续开发；
- 确认 `737d38e2d` 的调度池修复、进度账本和旧基线 `3/3` 均可追溯；
- 对新分支最终组合重跑受影响门槛和三轮连续无实现修改检查；
- 固定 phase-1 event fixture 和 migration 基线；
- scenemill 远端 main 合入该能力前，第二阶段只在新专用分支实施；后续同步 main 遵守非破坏性 merge
  与复验规则。
...
8. `web-studio` 子模块仓库先提交/推送，scenemill 仓库与 spring-ai-rag 仓库分别 merge 最新远端 main、复验、提交和 push；
