...
本轮 UX 收敛将 Files 定义为稳定的双栏文件工作区，而不是把 RAG 管理控件当作页面级导航
的一部分：

- 顶部全局命令栏只包含面包屑位置和刷新当前文件夹；不在根目录渲染一个看起来可操作但实际
  禁用的 RAG 区块，也不在目录切换时改变整页的行数和布局。
- 左侧区域明确标为“文件和文件夹”，使用当前目录列表语义；列表头、当前目录名称、文件类型/
  导入 UUID 次级信息和文件大小帮助用户扫描。UUID 继续作为稳定技术身份，但不再是唯一主标签。
- “在当前文件夹中查找”只筛选当前 API 返回的直接子项；进入文件夹、返回上一级或点击面包屑
  时清除局部筛选，避免把旧筛选带到另一个目录后产生误导性的空结果。
- 空目录和无匹配结果仍保留返回上一级入口。导入控件使用可聚焦的键盘按钮语义，拖放和点击
  都触发同一 PDF 导入流程。
- RAG 集合选择与“添加到 RAG”作为当前导入目录的上下文操作放在右侧当前文件夹摘要中；
  根目录不显示该操作，进入导入目录后也不会推动整个 Files 工作区或页面其他区域。
- 右侧没有选中文件时显示当前文件夹的名称、稳定路径和可用批次数量；选中文件后继续使用
  原有预览、刷新和打开原始文件动作。整个双栏工作区保持稳定高度和可调整的左栏宽度。
...
- 页面采用稳定的“位置/命令栏 + 目录列表 + 可调整分隔条 + 预览区”骨架；从根目录进入
  子目录时不得插入或移除整行区块、改变主体纵向起点或使预览区横向跳动；
- RAG Collection 和 Add-to-RAG 是上下文命令，放在右侧预览面板的当前导入上下文中；根目录
  不渲染该区块，进入 PDF 导入目录后也不能推动 Files 页面主体或全局命令栏；
- 目录列表宽度在桌面端可通过 pointer 拖动，也可在获得焦点后用方向键调整；宽度限制在
  `240–560px`，预览区至少保留 `320px`，并使用有界 session state 记住当前 tab 的选择；
- 分隔条使用 `role="separator"`、`aria-orientation="vertical"` 和数值属性，移动端切换为
  单列布局并隐藏分隔条；
- 根目录的双行可读名称/UUID 与目录内单行文件使用一致的固定行高和截断规则，内容差异不得
  改变面板几何；
- 查找输入使用本地 draft；中文等 IME 组合输入期间不得导航或改写 URL，在
  `compositionend` 后一次性提交 `q`，普通输入直接使用 `replace` 更新 URL。
