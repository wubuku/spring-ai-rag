...
- 2026-08-26：最终提交 `30ac001f` 上重新运行完整验收
  ...
  为干净状态。
- 2026-08-26：特性分支合并到 `main` 后，以合并提交
  `b511c648` 为基线重新执行完整验收 `preflight-main-post-merge-20260826`，仍为
  `16/16` 全部通过；真实服务 HTTP 合同 `129` 项和真实 API-key Playwright `1/1`
  均通过。该证据作为合并后最终结论，未沿用合并前结果。
