...
- a representative tenant/shared topology binds one read-only query principal
  ...
  independently, merges results deterministically, strips media/credential
  material from projections, rebuilds a browser-safe DTO, and preserves both
  bindings through query-credential rotation;
- client-owned mutation envelopes are compiled into stable hashed external
  identities, opaque revisions, retrieval text, and allow-listed JSON payloads
  before exercising a PROJECT CAS update, tombstone, restore, and post-rotation
  tombstone plus PLATFORM_SHARED publish/revoke; raw `mediaRef`, URLs, and
  internal candidate/event/fingerprint fields never enter the RAG projection;
...
`project-lifecycle-v1.json` through `project-lifecycle-v5.json` plus
`platform-lifecycle-v1.json` and `platform-lifecycle-v2.json`. The gate sends
only compiled safe projections to RAG and does not put the raw envelopes,
private media references, or the fixture path in the release manifest. With no
directory configured, self-contained fixtures using the same protocol are
