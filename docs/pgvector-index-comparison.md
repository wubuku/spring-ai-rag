...
rag:
  embedding:
    profile-key: siliconflow-bge-m3-1024-v1
    dimensions: 1024
    distance-metric: COSINE
```

```sql
-- Generated by EmbeddingProfileIndexManager for the active Profile
CREATE INDEX idx_rag_emb_p_<profile-id>_1024_hnsw
    ON rag_embeddings USING hnsw (embedding_1024 vector_cosine_ops)
    WHERE embedding_profile_id = <profile-id>;
```

The application-owned schema uses fixed-length `embedding_1024 VECTOR(1024)` and
an HNSW index per active Profile. The Profile predicate is part of the index so
the retrieval SQL can use an ANN index without mixing vector spaces. The old
`rag_vector_store` path is not an alternative configuration.

Changing the index algorithm or adding a new vector dimension requires a new
design and migration; it is not a runtime property switch.
...
- `V1__init_rag_schema.sql` — base vector schema
- `V25__embedding_profile_expand.sql` — Profile and fixed-dimension expansion
- `EmbeddingProfileIndexManager` — Profile-specific HNSW index creation

---

*Last reviewed: 2026-08-15 | spring-ai-rag 1.0.0 | pgvector 0.7.x*
