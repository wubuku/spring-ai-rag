...
└─────────────────────────────────────────────────────────────┘
    ...
    ▼
┌─────────────────────────────────────────────────────────────┐
│  缩放加权 RRF 融合（K=60，平滑常数）                         │
│  score(d) = (K+1) × Σ weight_s/(K + rank_s(d))             │
└─────────────────────────────────────────────────────────────┘
    │
    ▼
检索结果（topK，混合分数）
...
当前实现使用显式的、按通道分别配置的缩放加权 RRF。`K` 固定为 `60`，每个通道的权重
独立限制在有限的 `0.0..1.0`，服务不会自动归一化两个权重之和；通常建议配置
`vector-weight + fulltext-weight = 1.0`。提供方原始分数只用于确定通道内名次，不在
向量与全文通道之间直接比较；同一候选同时命中两个通道时，两个通道的贡献相加。
...
4. **缩放加权 RRF 融合公式**：`score(d) = (K+1) × Σ weight_s/(K + rank_s(d))`，K=60；
   同一候选命中多个通道时累加各通道贡献
