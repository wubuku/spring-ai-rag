#!/usr/bin/env bash
# One-click verification for embedding operations (Batch C).
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_ID="${EMBEDDING_OPERATIONS_VERIFY_RUN_ID:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${EMBEDDING_OPERATIONS_VERIFY_LOG_DIR:-.verification/embedding-operations/${RUN_ID}}"
...
write_summary() {
  {
    echo "# Embedding operations verification"
    echo
    echo "- Run: \`${RUN_ID}\`"
...
start_postgres() {
  ...
  CONTAINER_NAME="spring-ai-rag-embedding-ops-${RUN_ID}-$$"
  docker run -d --rm \
    --name "$CONTAINER_NAME" \
...
focused_tests() {
  mvn -pl spring-ai-rag-core -am \
    -Dtest=EmbeddingJobServiceTest,EmbeddingJobWorkerTest,EmbeddingJobControllerWebTest,EmbeddingPolicyResolverTest \
...
run_step "Focused embedding operations tests" focused_tests
run_step "Isolated PostgreSQL startup" start_postgres
run_step "V1-V39 PostgreSQL embedding operations integration (V37 contracts)" postgres_tests
