...
class SensitiveDataMaskingConverterTest {
    ...
    // ─── Batch 302 补充：脱敏模式覆盖扩展 ─────────────────────────

    @Test
    void masksJsonSecretField() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("{\"secret\":\"s3cr3t\"}");
        assertFalse(result.contains("s3cr3t"));
    }

    @Test
    void masksEscapedJsonPassword() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("\\\"password\\\":\\\"abc123\\\"");
        // 转义 JSON 模式
        assertTrue(result != null);
    }

    @Test
    void masksUrlTokenParameter() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("/api?token=abc123def&user=admin");
        assertFalse(result.contains("abc123def"));
        assertTrue(result.contains("user=admin"));
    }

    @Test
    void masksSqlDoubleQuotedPassword() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("password = \"s3cret\"");
    ...
    void masksAwsAccessKey() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("key=AKIAIOSFODNN7EXAMPLE");
        assertFalse(result.contains("AKIAIOSFODNN7EXAMPLE"));
    }
    ...
    void masksChineseNationalId() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("ID: 110101199003071234");
        assertFalse(result.contains("110101199003071234"));
    }

    @Test
    void masksChinesePhoneNumber() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("Phone: 13800138000");
        assertFalse(result.contains("13800138000"));
    }
    ...
    void normalTextPassesThroughUnchanged() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveData("Hello World 123");
    ...
    @Test
    void convertMasksFormattedMessage() {
        ILoggingEvent event = Mockito.mock(ILoggingEvent.class);
        Mockito.when(event.getFormattedMessage())
    ...
    @Test
    void convertNullMessageReturnsNull() {
        ILoggingEvent event = Mockito.mock(ILoggingEvent.class);
    ...
    @Test
    void maskSensitiveDataKeepTypeDetectsPassword() {
        String result = SensitiveDataMaskingConverter
    ...
    @Test
    void maskSensitiveDataKeepTypeDetectsBearerToken() {
        String result = SensitiveDataMaskingConverter
                .maskSensitiveDataKeepType("Bearer eyJhbGciOiJIUzI1NiJ9");
        assertTrue(result.contains("BEARER_TOKEN") || result.contains("TOKEN")
                || result.contains("SENSITIVE"));
    }

    @Test
    void maskSensitiveDataKeepTypeNullReturnsNull() {
        assertNull(SensitiveDataMaskingConverter.maskSensitiveDataKeepType(null));
    }

    @Test
    void multiplePatternsAreAppliedInOrder() {
        String input = "{\"password\":\"abc\"} Bearer eyJhbGciOiJIUzI1NiJ9";
        String result = SensitiveDataMaskingConverter.maskSensitiveData(input);
